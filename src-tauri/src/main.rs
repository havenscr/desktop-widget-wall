...
use windows::Win32::System::Com::{
    CoCreateInstance, CoInitializeEx, CLSCTX_ALL, COINIT_MULTITHREADED, STGM_READ, IPersistFile,
};
use windows::Win32::Storage::FileSystem::WIN32_FIND_DATAW;

// Windows Shell Link imports for Recent Files
use windows::Win32::UI::Shell::{IShellLinkW, ShellLink, SHGetFileInfoW, SHFILEINFOW, SHGFI_ICON, SHGFI_SMALLICON};
use windows::core::{PCWSTR, Interface};
use windows::Win32::Foundation::BOOL;
use windows::Win32::UI::WindowsAndMessaging::{GetIconInfo, ICONINFO, DestroyIcon};
use windows::Win32::Graphics::Gdi::{
    CreateCompatibleDC, DeleteDC, SelectObject, GetDIBits, DeleteObject,
...
fn start_audio_capture(
    ...
    thread::spawn(move || {
        println!("Starting audio capture: type={}, device={}, capture_id={}", device_type_clone, device_name_clone, new_capture_id);
        match run_audio_capture(&device_type_clone, &device_name_clone, buffer, new_capture_id) {
            Ok(_) => println!("Audio capture ended normally (ID={})", new_capture_id),
            Err(e) => eprintln!("Audio capture error (ID={}): {}", new_capture_id, e),
        }
    });

    Ok(format!("Started capturing from: {} (type: {}) [capture_id={}]", device_name, device_type, new_capture_id))
}
...
fn run_audio_capture(
    ...
    let device = if device_type == "input" {
        ...
        println!("Found output device for loopback: {:?}", output_device.name());
        output_device
    } else {
    ...
    println!("Audio config: sample_format={:?}, channels={}, sample_rate={}",
            ...
            device.build_input_stream(
                ...
                move |data: &[f32], _: &_| {
                    ...
                    if prev == 0 {
                        ...
                        println!("First audio batch: {} samples, max={:.4}", data.len(), max_val);
                    }
    ...
    println!("Audio stream started successfully, capturing from: {} (ID={})", device_name, capture_id);
    ...
    loop {
        ...
        if elapsed_ms < startup_immunity_ms as u128 {
            if current_id != capture_id {
                println!("Audio capture (ID={}) superseded by newer capture (ID={}), exiting", capture_id, current_id);
                break;
            }
        ...
        if stop_requested || current_id != capture_id {
            if current_id != capture_id {
                println!("Audio capture (ID={}) superseded by newer capture (ID={})", capture_id, current_id);
            } else {
...
fn convert_playback_status(status: GlobalSystemMediaTransportControlsSessionPlaybackStatus) -> PlaybackStatus {
    match status {
...
fn resolve_shortcut(lnk_path: &std::path::Path) -> Option<PathBuf> {
    unsafe {
        ...
        persist_file.Load(PCWSTR(wide_path.as_ptr()), STGM_READ).ok()?;
        ...
        // Convert to String
        let end = target_path.iter().position(|&c| c == 0).unwrap_or(target_path.len());
...
async fn get_recent_files(limit: Option<usize>) -> Vec<RecentFileInfo> {
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let recent_path = dirs::data_local_dir()
            .map(|p| p.join("..").join("Roaming").join("Microsoft").join("Windows").join("Recent"))
            .or_else(|| std::env::var("APPDATA").ok().map(|p| PathBuf::from(p).join("Microsoft").join("Windows").join("Recent")));
        ...
        let mut lnk_files: Vec<(PathBuf, std::time::SystemTime)> = entries
            ...
            .filter_map(|entry| {
                ...
                if path.extension().and_then(|e| e.to_str()) == Some("lnk") {
                    entry.metadata().ok().and_then(|m| m.modified().ok()).map(|modified| (path, modified))
                } else {
                    None
                }
            })
    ...
    tauri::async_runtime::spawn_blocking(move || {
        unsafe {
            ...
            *(ptr.add(20 + path_bytes) as *mut u16) = 0;
            ...
            let result = SetClipboardData(CF_HDROP.0 as u32, windows::Win32::Foundation::HANDLE(hglobal.0));
...
async fn open_folder_in_explorer(path: String) -> Result<(), String> {
    use windows::Win32::UI::WindowsAndMessaging::{
        EnumWindows, GetWindowTextW, SetWindowPos, SetForegroundWindow,
        IsWindowVisible, HWND_TOPMOST, HWND_NOTOPMOST, SWP_SHOWWINDOW, SWP_NOACTIVATE,
        ShowWindow, SW_RESTORE, SW_SHOW, BringWindowToTop,
        GetWindowThreadProcessId, GetForegroundWindow,
    };
    use windows::Win32::Graphics::Gdi::{
        GetMonitorInfoW, MONITORINFO, MonitorFromPoint, MONITOR_DEFAULTTOPRIMARY,
    };
...
#[tauri::command]
async fn open_volume_mixer(monitor: Option<u32>, width: Option<i32>, height: Option<i32>) -> Result<(), String> {
    use windows::Win32::UI::WindowsAndMessaging::{
        EnumWindows, GetWindowTextW, SetWindowPos, IsWindowVisible,
        HWND_TOP, SWP_SHOWWINDOW,
    };
    use windows::Win32::Graphics::Gdi::{
        GetMonitorInfoW, MONITORINFO, EnumDisplayMonitors, HDC, HMONITOR,
    };
    ...
    println!("[VolumeMixer] Requested: monitor={}, width={}, height={}", target_monitor_num, window_width, window_height);
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        unsafe {
            ...
            if let Some(hwnd) = FOUND_HWND {
                ...
                let _ = EnumDisplayMonitors(None, None, Some(volume_mixer_monitor_callback), LPARAM(0));
                ...
                for (idx, &mon) in MONITOR_LIST.iter().enumerate() {
                    ...
                    if GetMonitorInfoW(mon, &mut mi).as_bool() {
                        ...
                        println!("[VolumeMixer] Monitor {}: work area ({},{}) to ({},{}) primary={}",
                            idx + 1,
                ...
                if GetMonitorInfoW(target_monitor, &mut monitor_info).as_bool() {
                    ...
                    println!("[VolumeMixer] Positioning window at ({}, {}) with size {}x{}", x, y, window_width, window_height);
...
async fn open_meeting_on_primary(url: String) -> Result<(), String> {
    use windows::Win32::UI::WindowsAndMessaging::{
        GetForegroundWindow, SetWindowPos, SetForegroundWindow,
        HWND_TOP, SWP_NOSIZE, SWP_SHOWWINDOW, SWP_ASYNCWINDOWPOS,
    };
    use windows::Win32::Graphics::Gdi::{
        GetMonitorInfoW, MONITORINFO, MonitorFromPoint, MONITOR_DEFAULTTOPRIMARY,
    };
        ...
        unsafe {
            ...
            if session_name.to_lowercase() == "master" {
                let endpoint_volume: IAudioEndpointVolume =
                    device.Activate(CLSCTX_ALL, None).map_err(|e: windows::core::Error| e.to_string())?;
                endpoint_volume
            ...
            for i in 0..session_count {
                if let Ok(session_control) = session_enumerator.GetSession(i) {
                    ...
                    if let Ok(session_control2) = session_control.cast::<IAudioSessionControl2>() {
                        ...
                        if process_name.contains(&target_name) || target_name.contains(&process_name.replace(".exe", "")) {
                            if let Ok(volume_control) = session_control.cast::<ISimpleAudioVolume>() {
                                volume_control
            ...
            for i in 0..session_count {
                if let Ok(session_control) = session_enumerator.GetSession(i) {
                    ...
                    if let Ok(session_control2) = session_control.cast::<IAudioSessionControl2>() {
                        ...
                        if process_name.contains(&target_name) || target_name.contains(&process_name.replace(".exe", "")) {
                            if let Ok(volume_control) = session_control.cast::<ISimpleAudioVolume>() {
                                let current_mute = volume_control.GetMute().unwrap_or(BOOL(0)).as_bool();
...
fn chrono_lite_today() -> String {
    let now = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_secs() as i64;
    chrono_lite_format(now).split('T').next().unwrap_or("").to_string()
}
        ...
        while OAUTH_SERVER_RUNNING.load(Ordering::SeqCst) && start.elapsed() < timeout {
            ...
            match server.recv_timeout(Duration::from_millis(500)) {
                Ok(Some(request)) => {
                    ...
                    if url.starts_with("/auth-callback") || url.contains("code=") || url.contains("access_token=") {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if let Some(window) = app.get_window("main") {
                ...
                unsafe {
                    let _ = EnumDisplayMonitors(None, None, Some(startup_monitor_callback), LPARAM(0));
                }
                ...
                unsafe {
                    ...
                    if target_index < MONITOR_LIST.len() {
                        ...
                        if GetMonitorInfoW(MONITOR_LIST[target_index], &mut monitor_info).as_bool() {
                            ...
                            println!("[Startup] Positioning window on monitor 2 at ({}, {})", x, y);
